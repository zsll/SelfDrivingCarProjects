...
// To handle latency of 1s. Horizon T = 1s
size_t N = 10;
double dt = 0.1;
double ref_v = 60;  //ref speed is 60mph
...
class FG_eval {
 public:
  ...
  // Evaluate a 3rd order polynomial given by coeffs
  AD<double> evalPolynomial(const AD<double> x0) {
    AD<double> result = coeffs[0];
    for (int i = 1; i <= 3; i++) { // 3rd order polynomial
      result += coeffs[i] * pow(x0, i);
    }
    return result;
  }
  
  // Evaluate derivative a 3rd order polynomial given by coeffs
  AD<double> derivativePoly(const AD<double> &x0) {
    return coeffs[1] + (2 * coeffs[2] * x0) + (3 * coeffs[3] * pow(x0, 2));
  }
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      fg[0] += cte_cost_weight * CppAD::pow(vars[cte_start + t], 2);  // ref_cte is zero
      fg[0] += epsi_cost_weight * CppAD::pow(vars[epsi_start + t], 2); // ref_epsi is zero
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> f0 = evalPolynomial(x0);
      AD<double> psides0 = CppAD::atan(derivativePoly(x0));
...
static vector<double> getVecResult(CppAD::ipopt::solve_result<CPPAD_TESTVECTOR(double)> &solution) {
